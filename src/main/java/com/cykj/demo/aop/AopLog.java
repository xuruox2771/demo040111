...
public class AopLog {
    ...
    //	这个类似正则表达式,可以控制日志的精度(包下,类下,方法下)和切面的类型(业务层面,服务接口层面)相当灵活)
       //@Pointcut("execution(* com.cykj.demo.servlet.*Servlet.*(..))")
    @Pointcut("execution(* com.cykj.demo.controller.*Control.*(..))")
   //  @Pointcut("execution(* com.example.nba.repository.PlayerRep.*(..))")
    ...
    public void Mybefore(JoinPoint jp) {
        ...
        logger.info("请求的全类名 : " + jp.getSignature().getDeclaringTypeName() +
